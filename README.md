...
ComfyUI 视频生成任务的 Web 管理界面，支持通过浏览器提交视频生成任务、实时查看进度、管理素材与历史记录。

**当前版本：v1.5.0** （2026-08-28）

## ✨ v1.5.0 新特性

### 素材管理升级
- **文件管理弹窗预览**：点击文件卡片弹出预览窗口，支持图片/音频/视频/文本四种类型
- **一键添加到槽位**：在预览弹窗中直接将素材添加到指定参考槽位（图片 9 槽位、音视频各 3 槽位），带二次确认
- **文本添加到提示词**：文本文件可直接追加到提示词输入框
- **视频槽位缩略图**：视频参考槽位自动显示封面预览（服务器文件提取首帧，base64 文件通过 video 元素加载）
- **音频裁剪**：点击已选音频槽位可预览并设置裁剪区间，默认截取前 3 秒

### 默认预置素材
- **默认参考图**：卡通玩偶形象（`3d_hunyuan3d_multiview_to_model_front_image.png`）
- **默认参考视频**：广告示范视频（`ad_video_demo.mp4`）
- **默认提示词模板**：卡通玩偶动画场景，完整包含图片主体、音频音色、视频背景/运镜描述

### 智能体验优化
- **优雅 OOM 提示**：自动提取 GPU 显存占用信息，给出 ①②③④ 编号的优化建议，原始错误日志折叠在 `<details>` 中
- **视频时长步进**：改为 1 秒步进（原 0.5 秒），更符合实际使用习惯
- **界面精简**：移除生成按钮下方的空白行和冗余的视频播放器/下载按钮（统一在文件管理中预览下载）

### 稳定性与性能
- **素材去重**：上传文件采用内容 MD5 哈希命名，避免重复素材覆盖和磁盘浪费
- **输出路径修复**：SaveVideo 节点强制输出到 `Haimi/` 子目录，避免在根目录和子目录重复保存
- **避免重复下载**：ComfyUI 已保存到目标目录时，代理直接复用文件，不再重复下载保存
- **服务稳定性**：所有服务由 systemd 管理，异常自动重启
...
### 核心功能
- **视频生成**：通过 Web 界面提交视频生成任务，支持图片（最多 9 张）/ 音频（最多 3 条）/ 视频（最多 3 条）参考
- **实时进度**：显示 ComfyUI 节点级别的执行进度百分比
- **任务队列**：支持多任务排队，自动提交到 ComfyUI 队列，支持连续提交
- **任务状态**：待提交 → 排队中 → 处理中 → 完成/失败，失败任务可重试
- **中文支持**：完整支持中文文件名和中文提示词

### 素材管理
- **文件管理**：内置文件浏览器，根目录 `/mnt/storage`，支持浏览服务器全部存储内容
- **素材预览**：文件管理中支持图片、音频、视频、文本的弹窗预览
- **添加到槽位**：从文件管理直接选择素材添加到指定的图片/音频/视频参考槽位
- **添加到提示词**：文本文件可直接追加到提示词框
- **音频裁剪**：点击已选音频槽位，可预览并设置裁剪区间（默认前 3 秒），精确到 M:SS 格式
- **视频缩略图**：视频参考槽位显示视频封面预览（服务器文件自动提取首帧）
- **素材去重**：上传文件采用内容 MD5 哈希命名，避免重复素材占用磁盘

### 默认预置
- **默认参考图**：首次加载自动填充预置参考图（卡通玩偶形象）
- **默认参考视频**：自动填充预置参考视频（广告示范）
- **默认提示词**：自动加载匹配的提示词模板，包含图片主体、音频音色、视频背景/运镜完整描述

### 智能辅助
- **OOM 智能提示**：自动检测显存不足错误，提取实际显存占用信息，给出针对性优化建议
- **原始错误折叠**：详细错误日志折叠显示，不占用界面空间
- **优雅的错误处理**：网络错误、ComfyUI 未启动、超时等场景均有友好提示
- **输出路径统一**：所有生成视频自动保存到 `/mnt/storage/MMX_ComfyUI-output/Haimi/`，避免重复

### 其他
- **文件浏览器**：支持服务器素材浏览、选择
- **配置预设**：保存和加载常用配置组合
- **提示词模板**：内置多场景提示词模板库（含卡通玩偶、产品发布、情景喜剧等）
...
v1.5.0 起，系统会自动检测 OOM 错误并给出优化建议（含当前显存占用信息），原始错误日志折叠显示。
...
- [webui/static/prompt_guide.md](./webui/static/prompt_guide.md) — 提示词编写指南
- [workflow_configs/MiniMax_H3.json](./workflow_configs/MiniMax_H3.json) — 工作流配置说明

## 📝 版本

当前版本：**v1.5.0**（2026-08-28）

API 查询：`curl http://localhost:8025/api/version`
